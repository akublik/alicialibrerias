...
const generatePodcastScriptFlow = ai.defineFlow(
  ...
  async (input: GeneratePodcastScriptInput) => {
    ...
    const scriptResponse = await ai.generate({
        ...
        prompt: `Eres un guionista y presentador de podcasts literarios. Tu tarea es crear un guion para un episodio corto (2-3 minutos) sobre el libro "${input.bookTitle}" del autor ${input.authorName}, basándote en el contenido proporcionado.
...
2.  **Guion:** Escribe un guion completo que incluya:
    ...
    *   Un segmento sobre **el autor**: ¿Quién es ${input.authorName}? ¿Cuál es su estilo?
...
Genera solo el título y el guion.`,
    ...
    const generated = scriptResponse.output;

    if (!generated?.script || !generated?.title) {
        let missing = [];
        if (!generated?.title) missing.push("título");
        if (!generated?.script) missing.push("guion");
        throw new Error(`La IA no pudo generar un ${missing.join(' y ')} válido para el podcast.`);
    }

    const { script, title } = generated;

    // 2. Generate the audio from the script
    let audioUrl: string | undefined;
    try {
        const audioResponse = await textToSpeech({ text: script, voice: 'Achernar' });
