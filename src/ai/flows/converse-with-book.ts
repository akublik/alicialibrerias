...
const converseWithBookFlow: Flow<{ bookTitle: string; history: ChatMessage[] }, string> = defineFlow(
    {
        name: 'converseWithBookFlow',
    },
    async ({ bookTitle, history }) => {
...
export async function converseWithBook(bookTitle: string, history: ChatMessage[]): Promise<string> {
    return converseWithBookFlow({ bookTitle, history });
}
